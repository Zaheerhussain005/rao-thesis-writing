% !TEX program = xelatex
% !BIB program = bibtex
% ============================================================
%  main.tex  —  Root document
%  ML-Based Groundwater Potential Mapping of the Pothohar
%  Plateau, Punjab, Pakistan
%
%  Compile (Overleaf or local):
%     pdflatex main  →  bibtex main  →  pdflatex main  →  pdflatex main
% ============================================================
\documentclass[12pt,a4paper,oneside]{report}

\input{settings}

\begin{document}

% ---------------- Front matter (roman numerals) -------------
\pagenumbering{roman}

\input{preliminary/title_page}
\input{preliminary/certificate}
\input{preliminary/declaration}
\input{preliminary/dedication}
\input{preliminary/acknowledgement}
\input{preliminary/abstract}

\tableofcontents
\listoffigures
\listoftables
\input{preliminary/abbreviations}

\cleardoublepage

% ---------------- Main matter (arabic numerals) -------------
\pagenumbering{arabic}

\input{chapters/01_introduction}
\input{chapters/02_literature_review}
\input{chapters/03_methodology}
\input{chapters/04_results}
\input{chapters/05_discussion}
\input{chapters/06_conclusion}

% ---------------- References --------------------------------
\cleardoublepage
\addcontentsline{toc}{chapter}{References}
\bibliography{references}

% ---------------- Appendices --------------------------------
\appendix
\input{appendices/appendix_a_code}
\input{appendices/appendix_b_data}

\end{document}
